...
W celu analizy rozpatrujemy cene zamknięcia miedzi z okresu od 30 kwietnia 2020 do dania pisania tego raportu tj. 30 kwiecień 2023 roku. Dane prezentują się następująco.
```{r}
#getSymbols(Symbols="CPER" , src="yahoo", from='2013-04-12', to='2023-04-12')
getSymbols(Symbols="CPER" , src="yahoo", from='2020-04-30')
copper = CPER#$CPER.Close #tail(CPER, 300)
chartSeries(copper)
```
W naszych danych znajduję się `r sum(time(copper)[-1]-time(copper)[-length(copper)]==2)+sum(time(copper)[-1]-time(copper)[-length(copper)]>3)` brakujących danych (przy założeniu 5 dniowego tygodnia). Spowodowane jest to nie funkcjonowaniem giełdy w święta oraz wybrane dni. Daltego sporządziliśmy również wykres zmian tygodniowych, który wygląda następująco.
```{r}
chartSeries(to.weekly(copper))

```
Jednak do późniejszego obliczania wskaźników technicznych korzystaliśmy z danych dziennych. Na poniższym wykresie zamieściliśmy wszystkie wyniki dla podstawowych danych (według zaimplementowanych algorytmów). 
...
#candleChart(copper, TA=c(addBBands(), addMACD(),addSMI()))#, subset = '2018-01::')
chartSeries(copper, TA=c(addBBands(), addMACD(),addSMI(), addADX()), type="auto")#, subset = '2018-01::')



```
Na wykresie zamieszczone zostały wskaźniki takie jak: wstęga Bolingera, MACD, SMI oraz ADX.
...
\begin{itemize}
  \item \textbold{Wstęga bolingera} --  dwie czerwone i przerywane linie nałożone na pierwszy wykres. Jeśli cena spadnie poniżej dolnej linii jest to sygnał do kupna, jeśli przednie górną, to otrzymujemy sygnał do sprzedaży. Podczas analizy rozważymy zmianę dwóch parametrów w funkcji
  \begin{itemize}
  ...
  \end{itemize}
  \item \textbold{MACD} -- Moving Avrage Convergence Diverfence -- jest to wskaźnik dynamiki trendów, który pokazuje związek pomiędzy dwiema średnimi kroczącymi.
\end{itemize}
