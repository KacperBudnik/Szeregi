...
n=length(tmp)
inc.mean=TRUE
mat=sapply(0:5, function(p)
  sapply(0:5, function(q)
      tryCatch(arima(tmp,order=c(p,0,q), include.mean = inc.mean)$log, error=function(e) 0)))

k=sapply(0:5, function(p)
    sapply(0:5, function(q)
        1+p+q+inc.mean))
AIC=-2*mat + 2*k
...
colnames(AIC)=colnames(BIC)=colnames(AICc)=rownames(AIC)=rownames(BIC)=rownames(AICc)=0:5
...
#tmp=AIC
#c(floor(which(tmp==min(tmp))/6), which(tmp==min(tmp)) %% 6-1)


#?arima(tmp, )
#auto.arima(tmp, ic="bic")
#tmp.mat
# tmp=round(AIC, digits = 1)
# cell_spec(tmp,"latex",color = ifelse(tmp<= sort(tmp)[4], "blue", "black")) %>% kable("latex", escape = F)
# tmp
# tmp
# tmp %>%
#   cell_spec("latex",color = ifelse(tmp<= sort(tmp)[4], "blue", "black")) %>%
